...
class HomeController extends Controller
{
    ...
    {
        $user = Auth::user();
        $staff_online = User::select('users.id', 'username')->join('players', 'users.id', '=', 'players.user_id')->where('admin', '>', 1)->count();
        $players      = User::select('users.id', 'users.id', 'username', 'admin', 'avatar_url')->join('players', 'users.id', '=', 'players.user_id')->get();
